...
# 모델 로드
model = Model()
model.load_state_dict(torch.load("./weights/model_for_mnist.pth", weights_only=True))
model.to(device)
model.eval()
...
def pgd_targeted(model, x, target, eps, alpha=0.01, iters=40):
    ...
    for _ in range(iters):
        ...
        x_adv = torch.min(torch.max(x_adv, x - eps), x + eps)  # Projection
...
def pgd_untargeted(model, x, label, eps, alpha=0.01, iters=40):
    x_adv = x.clone().detach().to(device)
    label = label.to(device)
...
def plot_examples(eps, examples, title, attack_type):
    ...
    for i, (orig, adv, true_label, adv_label) in enumerate(examples):
        axes[0, i].imshow(orig.detach().cpu().squeeze(), cmap="gray")
        axes[0, i].set_title(f"Original: {true_label}", fontsize=10)
        axes[0, i].axis("off")

        axes[1, i].imshow(adv.detach().cpu().squeeze(), cmap="gray")
